  ...
  int run(int ls_cnt) {
    ...
    if (OB_FAIL(guard.switch_to(OB_SYS_TENANT_ID))) {
      ...
      for (auto iter = balance_part_job.bg_map_.begin(); iter != balance_part_job.bg_map_.end(); iter++) {
        ...
        const ObArray<ObPartitionBalance::ObLSPartGroupDesc*> &ls_part_groups = iter->second;
        for (int ls_idx = 0; ls_idx < ls_part_groups.count(); ls_idx++) {
          const ObPartitionBalance::ObLSPartGroupDesc* part_groups = ls_part_groups.at(ls_idx);
          ObSqlString part_groups_str;
          ObArray<ObTransferPartGroup *> part_groups_arr;
          for (int i = 0; i < part_groups->part_group_buckets_.count(); i++) {
            append(part_groups_arr, part_groups->part_group_buckets_.at(i));
          }
