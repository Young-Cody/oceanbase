    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < src_split_param.count(); ++i) {
      ...
      if (fabs(param.get_current_factor() - 1.0) < OB_DOUBLE_EPSINON) {
        ...
        if (param.get_ls_info()->ls_group_id_ == ls_status_info.ls_group_id_) {
          ...
          if (OB_FAIL(generate_ls_split_task_(param, task_index))) {
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    if (OB_FAIL(ObCommonIDUtils::gen_unique_id(tenant_id_, task_id))) {
      ...
      LOG_WARN("failed to construct ls part info", KR(ret), K(param));
                  ...
                  ls_group_id, param.get_ls_info()->ls_id_, dest_ls_id, part_list))) {
    ...
    const ObSplitLSParamArray &dest_split_param, const uint64_t ls_group_id)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    ObLSID dest_ls_id;
    if (OB_FAIL(generate_ls_split_task_(dest_split_param.at(0), task_begin_index))) {
    ...
    } else if (FALSE_IT(dest_ls_id = task_array_.at(task_begin_index).get_dest_ls_id())) {
    } else if (ls_group_id != task_array_.at(task_begin_index).get_ls_group_id()) {
      if (OB_FAIL(construct_ls_alter_task_(dest_ls_id, ls_group_id))) {
        LOG_WARN("failed to construct ls alter task", KR(ret), K(dest_ls_id), K(ls_group_id));
      }
    }
    for (int64_t i = 1; OB_SUCC(ret) && i < dest_split_param.count(); ++i) {
      ...
      } else if (ls_group_id != ls_info->ls_group_id_) {
        if (OB_FAIL(generate_ls_split_task_(split_param, task_begin_index))) {
          LOG_WARN("failed to generate ls split task", KR(ret), K(split_param));
        } else if (OB_UNLIKELY(task_begin_index < 0 || task_begin_index >= task_array_.count())) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("task_begin_index is invalid", KR(ret), K(task_begin_index));
        } else if (OB_FAIL(construct_ls_alter_task_(task_array_.at(task_begin_index).get_dest_ls_id(),
                                                    ls_group_id))) {
          LOG_WARN("failed to construct ls alter task", KR(ret),
                  K(task_array_.at(i).get_ls_group_id()), K(ls_group_id));
        } else if (OB_FAIL(construct_ls_merge_task_(task_array_.at(task_begin_index).get_dest_ls_id(),
                                                    dest_ls_id, ls_group_id))) {
          LOG_WARN("failed to construct ls merge task", KR(ret),
                  K(task_array_.at(task_begin_index).get_dest_ls_id()),
                  K(dest_ls_id), K(ls_group_id));
        }
      } else {
        if (OB_FAIL(generate_ls_transfer_task_(split_param, dest_ls_id, ls_group_id))) {
...
int ObLSBalanceTaskHelper::generate_ls_split_task_(const ObSplitLSParam &dest_split_param,
                                                  int64_t &task_begin_index)
{
  ...
  if (OB_UNLIKELY(!inited_)) {
    ...
    } else if (OB_FAIL(construct_ls_part_info_(dest_split_param, part_list))) {
